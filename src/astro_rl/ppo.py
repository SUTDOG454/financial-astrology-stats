...
class RolloutBuffer:
    ...
    def flatten(self, adv, returns):
        ...
        return {
            "market": self.market[:self.ptr].reshape(n, -1),
            "astro": self.astro[:self.ptr].reshape(n, -1),
            "actions": self.actions[:self.ptr].reshape(n, -1),
            "regimes": self.regimes[:self.ptr].reshape(n),
            "old_logp": self.logp[:self.ptr].reshape(n),
            "old_values": self.values[:self.ptr].reshape(n),
...
def regime_target_from_market(market: torch.Tensor, regime_dim: int) -> torch.Tensor:
    """Pseudo-labels from contemporaneous state only; deliberately no future returns."""
    vol = market[:, 2].abs()
    trend = market[:, 1].abs()
    score = torch.sigmoid(50.0 * vol) + 0.5 * torch.sigmoid(5.0 * trend)
    ...
    def update(self, batch, advantages, returns):
        ...
        for _ in range(self.cfg.epochs):
            ...
            for start in range(0, n, self.cfg.minibatch_size):
                ...
                pg1 = ratio * adv[idx]
                pg2 = ratio.clamp(1.0 - self.cfg.clip_eps, 1.0 + self.cfg.clip_eps) * adv[idx]
